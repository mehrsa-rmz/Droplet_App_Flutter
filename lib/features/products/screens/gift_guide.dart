    ...
    setState(() {
      _filteredProducts = _products.where((product) {
        ...
        bool matchesSex = widget.sex == 'all'
            ? true
            : product['sex'] == widget.sex || product['sex'] == 'all';
        bool matchesAge = widget.age == 'all'
            ? true
            : product['age'] == widget.age || product['age'] == 'all';
...
Widget genderToggleButton(
  ...
  return Container(
    ...
    decoration: BoxDecoration(
      borderRadius: BorderRadius.circular(8),
      color: isSelected ? blue7 : Colors.transparent,
    ),
    child: TextButton(
      style: TextButton.styleFrom(
        minimumSize: Size.zero,
      ...
      onPressed: onPressed,
      child: Text(title,
          style: tMenu.copyWith(color: isSelected ? white1 : blue7)),
    ),
  );
}
...
class _FilterAndSortWidgetState extends State<FilterAndSortWidget> {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            Text('Age group', style: tFilter.copyWith(color: black)),
            const SizedBox(height: 12),
            Wrap(
              spacing: 12,
              runSpacing: 12,
  ...
  List<Widget> _buildAppliedFilters() {
    ...
    filters.add(_buildFilterChip(_age, () {
      setState(() {
        _age = 'all';
      });
      widget.onAgeChanged('all');
    }));
  ...
  Widget _buildAgeButton(String title) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 3),
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(8),
        color: _age == title ? grey1 : white1,
        border: Border.all(color: grey8, width: 2),
      ),
      child: TextButton(
        style: TextButton.styleFrom(
